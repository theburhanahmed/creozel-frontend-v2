...
import { MicIcon, SparklesIcon, DownloadIcon, SaveIcon, PlayIcon, PauseIcon, Volume2Icon, VolumeXIcon } from 'lucide-react';
...
export const AudioEditor = () => {
  ...
  const [selectedTool, setSelectedTool] = useState<string | null>(location.state?.selectedTool || 'text-to-speech');
  ...
  const renderResult = () => {
    ...
    return <Card className="mt-6">
        <div className="p-6">
          ...
          {/* Audio Player */}
          {result.audioUrl && <div className="space-y-4">
              <div className="p-6 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-lg border border-amber-200 dark:border-amber-800/30">
                ...
                <div className="mb-4 h-20 flex items-end gap-1 justify-center">
                  {Array.from({
                length: 50
              }).map((_, i) => <div key={i} className="w-1 bg-gradient-to-t from-amber-500 to-orange-500 rounded-full transition-all duration-300" style={{
                ...
                <div className="mb-4">
                  <div className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-amber-500 to-orange-500 transition-all duration-300" style={{
                ...
                <div className="flex items-center justify-center gap-4">
                  <button onClick={toggleMute} className="w-10 h-10 rounded-full bg-white/50 dark:bg-gray-800/50 flex items-center justify-center hover:bg-white/70 dark:hover:bg-gray-800/70 transition-colors">
                    {isMuted ? <VolumeXIcon size={20} className="text-gray-700 dark:text-gray-300" /> : <Volume2Icon size={20} className="text-gray-700 dark:text-gray-300" />}
                  </button>
                  <button onClick={togglePlayPause} className="w-16 h-16 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 flex items-center justify-center hover:from-amber-600 hover:to-orange-600 transition-all shadow-lg hover:shadow-xl active:scale-95">
                    {isPlaying ? <PauseIcon size={28} className="text-white" /> : <PlayIcon size={28} className="text-white ml-1" />}
                  </button>
                  <button className="w-10 h-10 rounded-full bg-white/50 dark:bg-gray-800/50 flex items-center justify-center hover:bg-white/70 dark:hover:bg-gray-800/70 transition-colors">
                    <Volume2Icon size={20} className="text-gray-700 dark:text-gray-300" />
                  </button>
                </div>
              </div>
              {/* Audio Details */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {result.voice && <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
                ...
                {result.genre && <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">
                      Genre
                    </div>
                    <div className="text-sm font-medium text-gray-900 dark:text-white">
                ...
                {result.mood && <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">
                      Mood
                    </div>
                    <div className="text-sm font-medium text-gray-900 dark:text-white">
          ...
          {result.transcription && <div className="space-y-4">
  ...
  return <div className="flex gap-6 h-[calc(100vh-8rem)]">
      <ContentToolsSidebar currentCategory="audio" selectedTool={selectedTool} onSelectTool={setSelectedTool} />
